...
static void test_all_classes_serialize(void **state) {
  ...
  JdwpLibError e = all_classes_serialize(
      &buf, &bytes_written, &cmd, JDWP_VIRTUAL_MACHINE_ALL_CLASSES, NULL, 1);
...
static void test_all_classes_deserialize(void **state) {
  ...
  JdwpReply *reply;
  size_t len;
  JdwpLibError e = all_classes_deserialize(
      &reply, &len, vm_reply, JDWP_VIRTUAL_MACHINE_ALL_CLASSES, &id_sizes);
