...
void reply_callback(JdwpReply *reply, void **state) {
  ...
  assert_true(data->object_id_size > 0);
  assert_true(data->field_id_size > 0);
  assert_true(data->method_id_size > 0);
  assert_true(data->reference_type_id_size > 0);
  assert_true(data->object_id_size > 0);
...
static void test(void **state) {
  ...
  err = jdwp_client_send(client, 107, JDWP_VIRTUAL_MACHINE_ID_SIZES, NULL);
  assert_int_equal(err, JDWP_LIB_ERR_NONE);
